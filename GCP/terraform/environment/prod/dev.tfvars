####PROJECT
project_id              = "test-production-env"
environment             = "prod"

#######NETWORK
####VPC
region                  = "asia-south1"
vpc_name                = "test-prod-vpc"

####SUBNET
subnet_name             = "test-pri-subnet1"
subnet_cidr             = "10.20.0.0/20"
region_or_zone_name     = "asia-south1"     # for regional cluster

public_subnet_1_cidr = "10.20.16.0/20"
public_subnet_1_name = "test-public-subnet-1"

public_subnet_2_name = "test-public-subnet-2"
public_subnet_2_cidr = "10.20.32.0/20"

public_subnet_3_name = "test-public-subnet-3"
public_subnet_3_cidr = "10.20.48.0/20"

private_subnet_1_name = "test-private-subnet-1"
private_subnet_1_cidr = "10.20.64.0/20"

private_subnet_2_name = "test-private-subnet-2"
private_subnet_2_cidr = "10.20.80.0/20"



####NAT
nat_ip_address_name = "test-nat-ip"
test_nat_name = "test-nat-gateway"
test_nat_router_region = "asia-south1"

####GKE
cluster_name            = "test-prod-gke-cluster"
node_pool_name          = "test-node-pool"
machine_type            = "c2d-standard-2"
node_count              = 1
enable_private_nodes    = true
enable_private_endpoint = false
master_authorized_networks = [
  {
    cidr_block   = "192.168.1.0/24"
    display_name = "corporate-office"
  },
  {
    cidr_block   = "14.141.60.58/32"
    display_name = "sela-office-01"
  },
  {
    cidr_block   = "182.72.58.210/32"
    display_name = "sela-office-02"
  }
]

master_ipv4_cidr_block = "172.16.0.0/28"

test_secondary_ranges        = [
  {
    range_name = "pods"
    ip_cidr_range = "10.1.0.0/16"
  },
  {
    range_name = "services"
    ip_cidr_range = "10.2.0.0/16"
  }
]


####Node Pool
##test node pool
plnp_node_pool_name = "test-node-pool"
plnp_machine_type   = "c2d-standard-2"

# #Namespace
# #provider = "kubernetes"
# namespace_finbingo = "finbingo"
# namespace_test = "test"
# namespace_nginx = "nginx"


##############################################################################################################################

vm_1 = {
  machine_name = "test-jump-server"
  machine_type  = "e2-medium"
  image_family  = "ubuntu-2204-lts"
  image_project = "ubuntu-os-cloud"
  network       = "test-vpc"
  subnetwork    = "test-public-subnet-1"  
}

vm_2 = {
  machine_name  = "test-redis"
  machine_type  = "n2d-custom-2-4096"   # 2 vCPUs, 4096 MB RAM (4 GB)
  image_family  = "ubuntu-2204-lts"
  image_project = "ubuntu-os-cloud"
  network       = "test-vpc"
  subnetwork    = "test-pri-subnet1"
  zone          = "asia-south1-b"
}

vm_3 = {
  machine_name  = "test-masterdata-db"
  machine_type  = "n2d-custom-2-4096"     # 2 vCPUs, 4096 MB RAM (4 GB)
  image_family  = "ubuntu-2204-lts"
  image_project = "ubuntu-os-cloud"
  network       = "test-vpc"
  subnetwork    = "test-pri-subnet1"
  zone          = "asia-south1-c"
}


vm_4 = {
  machine_name  = "test-rmapi-logging-db"
  machine_type  = "n2d-custom-2-4096"     # 2 vCPUs, 4096 MB RAM (4 GB)
  image_family  = "ubuntu-2204-lts"
  image_project = "ubuntu-os-cloud"
  network       = "test-vpc"
  subnetwork    = "test-pri-subnet1"
  zone          = "asia-south1-c"
}


vm_5 = {
  machine_name  = "test-memcache"
  machine_type  = "n2d-custom-2-4096"     # 2 vCPUs, 4096 MB RAM (4 GB)
  image_family  = "ubuntu-2204-lts"
  image_project = "ubuntu-os-cloud"
  network       = "test-vpc"
  subnetwork    = "test-pri-subnet1"
  zone          = "asia-south1-c"
}


vm_6 = {
  machine_name  = "test-ondc-mongo-db"
  machine_type  = "n2d-custom-2-4096"     # 2 vCPUs, 4096 MB RAM (4 GB)
  image_family  = "ubuntu-2204-lts"
  image_project = "ubuntu-os-cloud"
  network       = "test-vpc"
  subnetwork    = "test-pri-subnet1"
  zone          = "asia-south1-c"
}
################################################################################################################################

#region      = "us-central1"
repo_id     = "test-prod-repo"
#environment = "prod"

#project_id       = "your-project-id"
sa_name          = "artifact-registry-sa"
sa_display_name  = "Artifact Registry SA"



db_instance_name   = "test-postgres-db"
db_tier            = "db-custom-2-4096"       # 2 vCPU, 4 GB RAM
db_version         = "POSTGRES_16"
db_user            = "postgres"
db_password        = "t9Q#G3v7eR$gW2wj"
db_name            = "test-postgres-db"
db_private_network = "projects/test-production-env/global/networks/test-prod-vpc"
disk_size          = 30
disk_type          = "PD_SSD"


